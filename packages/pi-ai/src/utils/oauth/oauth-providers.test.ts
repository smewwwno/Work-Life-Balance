...
describe("OAuth credentials — de-obfuscated regression", () => {
	...
	test("Antigravity CLIENT_ID and CLIENT_SECRET are plaintext", () => {
		...
		assert.ok(
			content.includes(
				'CLIENT_ID = "YOUR_ANTIGRAVITY_CLIENT_ID.apps.googleusercontent.com"',
			),
		);
		assert.ok(
			content.includes('CLIENT_SECRET = "GOCSPX-YOUR_ANTIGRAVITY_CLIENT_SECRET"'),
		);
		// Should NOT contain atob calls for de-obfuscation
		assert.ok(!content.includes("atob("));
	});
	...
	test("Gemini CLI CLIENT_ID and CLIENT_SECRET are plaintext", () => {
		...
		assert.ok(
			content.includes(
				'CLIENT_ID = "YOUR_GEMINI_CLIENT_ID.apps.googleusercontent.com"',
			),
		);
		assert.ok(
			content.includes('CLIENT_SECRET = "GOCSPX-YOUR_GEMINI_CLIENT_SECRET"'),
		);
		// Should NOT contain atob calls for de-obfuscation
		assert.ok(!content.includes("atob("));
	});
